...
export default function MobileNav(){
    ...
    return (
        <Sheet>
            ...
            <SheetContent className="space-y-3 bg-slate-50 py-4 mx-4 border-orange-500">
                <SheetTitle>
                  {
                    isAuthenticated ? (
                        <span className="flex items-center font-bold gap-2 mx-4 hover:text-orange-500">
                            <CircleUserRound className="text-orange-500" />
                            {user?.email}
                        </span>
                    ):(
                        <span className="flex items-center font-bold gap-2 mx-4">
                            Bienvenidos a FoodAppItz.com </span>
                    )
                  }
                </SheetTitle>
                <Separator/>
